...
function Recordings() {
  ...
  const reprocessRecording = (id: string) => {
    ...
    setTimeout(() => {
      setRecordings(prev => prev.map(recording => 
        recording.id === id ? { ...recording, status: 'processed' } : recording
      ));
      ...
      if (file) {
        ...
        setTimeout(() => {
          setRecordings(prev => prev.map(rec => 
            rec.id === newRecording.id ? { ...rec, status: 'processed', duration: '3:45' } : rec
          ));
  ...
  return (
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <nav className="hidden md:ml-8 md:flex space-x-1">
                ...
                <button onClick={() => navigate('/recordings')} className="text-slate-700 bg-red-50 px-4 py-2 rounded-lg text-sm font-medium">Recordings</button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Voice Recordings</h1>
          <p className="text-slate-600">{recordings.length} recordings processed by AI</p>
        </div>
            ...
            <div className="flex gap-2">
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium ${
                  filter === 'all' ? 'bg-red-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
                All
              </button>
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium ${
                  filter === 'processed' ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
                Processed
              </button>
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium ${
                  filter === 'processing' ? 'bg-yellow-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
                Processing
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRecordings.map((recording) => (
            <div key={recording.id} className="bg-white rounded-lg border border-slate-200 p-6 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    recording.source === 'voice' ? 'bg-red-100 text-red-600' :
                    recording.source === 'call' ? 'bg-blue-100 text-blue-600' :
                    'bg-green-100 text-green-600'
                  }`}>
                    <span className="text-lg">
                      {recording.source === 'voice' ? '🎙️' : recording.source === 'call' ? '📞' : '📁'}
                    </span>
                  </div>
                  <div>
                ...
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                  recording.status === 'processed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {recording.status}
                </span>
              </div>
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex justify-between text-sm text-slate-600">
                  <span>Source</span>
                  <span className="font-medium capitalize">{recording.source}</span>
                </div>
              </div>
        ...
        {filteredRecordings.length === 0 && (
          <div className="text-center py-12">
            ...
            <div className="text-slate-500 text-sm">Try uploading a recording or adjusting your filters</div>
            <button 
              onClick={uploadRecording}
